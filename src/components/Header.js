...
const Bar1 = styled.div`
  ...
  margin: auto;
  transform: ${prop =>
    prop.open ? 'rotate(-45deg) translate(-8px, 7.5px)' : undefined};
`
...
const Bar3 = Bar1.extend`
  transform: ${prop =>
    prop.open ? 'rotate(45deg) translate(-7.5px, -8px)' : undefined};
`
...
export default class Header extends React.PureComponent {
  ...
  render() {
    const links = pages.map(page => {
      ...
      return (
        <ListItem key={page}>
          <Navlink exact onClick={this.toggleDropdown} to={to}>
            {title}
          </Navlink>
        </ListItem>
      )
    ...
    return (
      <HeaderWrapper>
        <Circle>
          <BarWrapper>
            <Bar1 open={this.state.openDropdown} />
            <Bar2 open={this.state.openDropdown} />
            <Bar3 open={this.state.openDropdown} />
          </BarWrapper>
        </Circle>
